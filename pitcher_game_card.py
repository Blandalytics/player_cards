...
def arm_angle_day(game_date,season=2026):
    ...
    elif (szn_start - game_date).days >= 0:
...
def szn_games(player_id,game_id,season=None,sport_id=1):
    ...
    response = requests.get(url=f'http://statsapi.mlb.com/api/v1/people/{player_id}?hydrate=stats(group=pitching,type=gameLog,season={season},startDate={start_date},endDate={end_date},sportId=1,gameType=[R]),hydrations').json()
...
def header_stats_chunk(game_id,pitcher_id,ax):
    ...
    if ((starter==0) & (outs<11)) | (tbf<=6):
        ...
        game_score = 50 + (int(outs)-int(strikeouts)) * win_weight_dict['out'][(inning,run_diff)] + int(strikeouts) * win_weight_dict['strikeout'][(inning,run_diff)] + int(walks) * win_weight_dict['walk'][(inning,run_diff)] + int(hits) * win_weight_dict['hit'][(inning,run_diff)] + int(home_runs) * win_weight_dict['home_run'][(inning,run_diff)]
        grade_edges = [-1000,27,37,42,46,48,50,51,52,53,56,61,67,1000]
...
pitchtype_metrics_dict = {
    'FF':{
        ...
        'plvStuff+':[-10e6,80,88,105,114,10e6],
    ...
    'SI':{
        ...
        'plvStuff+':[-10e6,69,74,83,95,10e6],
    ...
    'FC':{
        ...
        'plvStuff+':[-10e6,90,107,115,125,10e6],
    ...
    'SL':{
        ...
        'plvStuff+':[-10e6,95,100,110,125,10e6],
    ...
    'ST':{
        ...
        'plvStuff+':[-10e6,105,111,120,130,10e6],
    ...
    'CU':{
        ...
        'plvStuff+':[-10e6,91,100,113,125,10e6],
    ...
    'CH':{
        ...
        'plvStuff+':[-10e6,100,107,115,125,10e6],
    ...
    for pitch_type in pitch_list:
        ...
        for stat in stat_list:
            ...
            if (stat == 'Velo') & vs_past:
                ...
                if (stat in ['Velo','SwStr%','CSW%','xSLGcon','PLV+','plvStuff+']) & (pitch_type in pitchtype_metrics_dict.keys()):
                    if np.isnan(stat_val):
                        stat_color = 'w'
                        color_val = 2
                    else:
...
def generate_chart(pitcher_id,game_id,game_df,game_group,szn_df,szn_comp,prev_season,comp_year):
    ...
    start_grade_ax.text(0.5,0.8,code_map[game_code],ha='center',va='center',fontsize=20,color=pl_line_color)
    ...
    chart_lim = max(29,int(game_df[['HB','IVB']].abs().max().max()/6+2)*6-1)
    ...
    for dist in range(1,int((chart_lim+6)/12)+1):
        dist = dist * 12
    ...
    sns.scatterplot(data=(game_df.loc[(game_df['hitterHand']=='L')].assign(pX = lambda x: np.clip(x['pX']*-1,-2,2),
                                                                           sz_plot_z = lambda x: np.clip(x['sz_plot_z'],-0.25,5.25))),
                    x='pX',
                    y='sz_plot_z',
                    hue='pitchType',
                    palette=marker_colors,
    ...
    if comp_year:
        if prev_season:
            ...
            fig.text(0.02,0.285,f'Shaded Regions are\nprior {comp_year} Shapes\ntranslated to current Velo',alpha=0.5)
    ...
    if game_df.loc[game_df[['sz_top','sz_bot','velo','extension',
        ...
        fig.text(0.632,0.2575,'*',fontsize=18,va='center',ha='center',alpha=0.5)
...
with col1:
    ...
    date_r = requests.get(f'https://statsapi.mlb.com/api/v1/schedule?sportId=1,14,21,51&date={ss['date']}')
    date_x = date_r.json()
